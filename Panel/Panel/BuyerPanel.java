...
public class BuyerPanel extends JPanel {
    ...
    public BuyerPanel(JFrame frame, User buyer) {
        ...
        add(seedScroll, BorderLayout.CENTER);
        ...
        JButton tradeButton = createStyledButton("Request Trade (All)", new Color(0, 150, 100));
        JButton viewTradesBtn = createStyledButton("View Trade Requests", new Color(255, 165, 0));
        JButton logoutButton = createStyledButton("Logout", new Color(200, 60, 60));

        bottom.add(buyButton);
        bottom.add(tradeButton);
        bottom.add(viewTradesBtn);
        bottom.add(logoutButton);
        add(bottom, BorderLayout.SOUTH);

        // --- Actions ---
        buyButton.addActionListener(e -> buySelectedSeed());
        tradeButton.addActionListener(e -> requestTradeForAll());
        viewTradesBtn.addActionListener(e -> openTradeRequestsWindow());
    ...
    private void loadAvailableSeeds() {
        ...
        List<Seed> list = buyerDAO.getAllAvailableSeeds();
    ...
    // --- Bulk trade request for ALL available seeds ---
    private void requestTradeForAll() {
        String offer = JOptionPane.showInputDialog(this,
                "Enter what you want to offer in exchange for ALL available seeds/products:");
        ...
        boolean success = buyerDAO.requestTradeForAll(buyer.getId(), offer);
        if (success) {
            JOptionPane.showMessageDialog(this, "Trade requests submitted for all available seeds!");
            loadAvailableSeeds();
        } else {
            JOptionPane.showMessageDialog(this, "Failed to submit trade requests.");
        }
    }

    // --- Open Trade Requests in a separate window ---
    private void openTradeRequestsWindow() {
        JFrame tradeFrame = new JFrame("Trade Requests");
        tradeFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        tradeFrame.setContentPane(new TradeRequestsPanel(tradeFrame, buyer.getId()));
        tradeFrame.pack();
        tradeFrame.setLocationRelativeTo(frame);
