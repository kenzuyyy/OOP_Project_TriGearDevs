    ...
    private void addSeed() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                if (!farmerDAO.isValidSeed(name)) {
                    JOptionPane.showMessageDialog(this, "Invalid seed/product name. Please enter a valid seed.");
                    return;
                }

                boolean success = farmerDAO.addSeed(farmer.getId(), name, qty, status, trade);
                if (success) {
    ...
    private void editSelectedSeed() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                // ✅ Seed/product validation
                if (!farmerDAO.isValidSeed(newName)) {
                    JOptionPane.showMessageDialog(this, "Invalid seed/product name. Please enter a valid seed.");
                    return;
                }

                boolean success = farmerDAO.updateSeed(seedId, newName, newQty, newStatus, newTrade);
                if (success) {
